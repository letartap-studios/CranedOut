...
#define     PHYSAC_VECTOR_ZERO          { 0.0f, 0.0f }
...
static double GetCurrentTimeRay(void);                                                                         // Get current time measure in milliseconds
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(Vector2 pos, float width, float height, float density)
{
	...
	if (newId != -1)
	{
		...
		newBody->shape.transform = Mat2Radians(0.0f);
		newBody->shape.vertexData = CreateRectanglePolygon(pos, { width, height });
			...
			if (collision)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					for (int j = 0; j < newData.vertexCount; j++)
					{
						...
						newData.normals[j] = { face.y, -face.x };
...
static PolygonData CreateRectanglePolygon(Vector2 pos, Vector2 size)
{
	...
	// Calculate polygon vertices positions
	data.positions[0] = { pos.x + size.x / 2, pos.y - size.y / 2 };
	data.positions[1] = { pos.x + size.x / 2, pos.y + size.y / 2 };
	data.positions[2] = { pos.x - size.x / 2, pos.y + size.y / 2 };
	data.positions[3] = { pos.x - size.x / 2, pos.y - size.y / 2 };
...
PHYSACDEF void RunPhysicsStep(void)
{
	...
	currentTime = GetCurrentTimeRay();

	// Calculate current delta time
	double delta = currentTime - startTime;

	delta = max(delta, 1.f / 30.f);
...
static void SolveCircleToCircle(PhysicsManifold manifold)
{
	...
	else
	{
		...
		manifold->normal = { normal.x / distance, normal.y / distance }; // Faster than using MathNormalize() due to sqrt is already performed
		manifold->contacts[0] = { manifold->normal.x * bodyA->shape.radius + bodyA->position.x, manifold->normal.y * bodyA->shape.radius + bodyA->position.y };
	}
...
static void SolveDifferentShapes(PhysicsManifold manifold, PhysicsBody bodyA, PhysicsBody bodyB)
{
	...
	if (separation < PHYSAC_EPSILON)
	{
		...
		manifold->normal = { -normal.x, -normal.y };
		manifold->contacts[0] = { manifold->normal.x * bodyA->shape.radius + bodyA->position.x, manifold->normal.y * bodyA->shape.radius + bodyA->position.y };
		manifold->penetration = bodyA->shape.radius;
		return;
	}
...
static void SolvePolygonToPolygon(PhysicsManifold manifold)
{
	...
	if (Clip({ -sidePlaneNormal.x, -sidePlaneNormal.y }, negSide, &incidentFace[0], &incidentFace[1]) < 2)
		return;
	...
	Vector2 tempVect = { -refFaceNormal.x, -refFaceNormal.y };

	manifold->normal = (flip ? tempVect : refFaceNormal);
...
static void InitializePhysicsManifolds(PhysicsManifold manifold)
{
	...
	for (int i = 0; i < manifold->contactsCount; i++)
	{
		...
		if (MathLenSqr(radiusV) < (MathLenSqr({ gravityForce.x * (float)deltaTime / 1000, gravityForce.y * (float)deltaTime / 1000 }) + PHYSAC_EPSILON))
	...
	for (int i = 0; i < manifold->contactsCount; i++)
	{
		...
		if (bodyA->enabled)
		{
			...
			if (!bodyA->freezeOrient)
				bodyA->angularVelocity += bodyA->inverseInertia * MathCrossVector2(radiusA, { -impulseV.x, -impulseV.y });
		}
		...
		if (absImpulseTangent < impulse * manifold->staticFriction)
			tangentImpulse = { tangent.x * impulseTangent, tangent.y * impulseTangent };
		else
			tangentImpulse = { tangent.x * -impulse * manifold->dynamicFriction, tangent.y * -impulse * manifold->dynamicFriction };
		...
		if (bodyA->enabled)
		{
			...
			if (!bodyA->freezeOrient)
				bodyA->angularVelocity += bodyA->inverseInertia * MathCrossVector2(radiusA, { -tangentImpulse.x, -tangentImpulse.y });
		}
...
static float FindAxisLeastPenetration(int* faceIndex, PhysicsShape shapeA, PhysicsShape shapeB)
{
	...
	for (int i = 0; i < dataA.vertexCount; i++)
	{
		...
		Vector2 support = GetSupport(shapeB, { -normal.x, -normal.y });
...
static void InitTimer(void)
{
	...
	startTime = GetCurrentTimeRay();   // Get current time
}
...
static double GetCurrentTimeRay(void)
{
	double a = (double)(GetTimeCount() - baseTime) / frequency * 1000;
	return a;
}
...
{
	return { -value * vector.y, value * vector.x };
}
...
{
	return { matrix.m00 * vector.x + matrix.m01 * vector.y, matrix.m10 * vector.x + matrix.m11 * vector.y };
}
